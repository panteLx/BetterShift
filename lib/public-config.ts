...
export async function getPublicConfig() {
  const guestAccess = await allowGuestAccess();
  ...
  return {
    ...
    auth: {
       ...
       * Live, admin-configurable via /admin (see lib/system-settings.ts);
       * ALLOW_GUEST_ACCESS is only the seed value for the first DB row.
       * @default false
       */
