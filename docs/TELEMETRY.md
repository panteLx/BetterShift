...
Admins can also send the payload right away with the **Send now** button in Admin → System Settings instead of waiting for the daily timer. It follows the same rules as the daily send — it only works while telemetry is on, and the consent covers the current `schemaVersion` — and it does not reset the daily schedule. It is rate limited (default 5 per hour, `RATE_LIMIT_TELEMETRY_SEND_REQUESTS` / `RATE_LIMIT_TELEMETRY_SEND_WINDOW`) and leaves an audit-log entry with only the outcome, never the endpoint URL. Every automatic send leaves the same kind of entry (`system.telemetry_send`), so the last outcome is visible in the audit log even though the timer itself stays silent.
...
| `app.isDev`                      | `false`                                            | Is this a development build. Dev builds send like any other; the flag only lets the receiving side tell them apart.                                                                |
...
- **Publishes aggregated statistics on a public page**, at `GET /` on the hub's URL (`https://telemetry-bettershift.pantelx.com` by default), with the same figures as JSON at `GET /data.json`. Both are computed hourly by a scheduled job (`src/aggregate.ts`) and served from that precomputed result, so the page never queries the raw ping table directly.
- **Only aggregates appear there** — counts and shares grouped across instances. The published data contains no instance id, no raw row, and no per-instance timestamp; the closest thing to a timestamp is `computedAt`, the time the hourly job last ran, which is not tied to any individual instance.
- **There is no small-group threshold.** A value reported by only one instance — a Node version nobody else is on, a timezone unique to one deployment — is published as such, with an instance count of `1`. An earlier design collapsed small groups into an "other" bucket; the maintainer removed it, since the previous telemetry backend had no such floor either. If you are the only instance reporting a given value, that fact is visible on the public page. The one size limit that does exist is unrelated to this: each breakdown lists at most its 25 largest values and sums the rest into one `remainder`-marked entry, purely so a page with an attacker-controlled, unbounded field (custom field types) cannot grow without limit — it is not a privacy measure, and nothing is hidden for being rare.
- **Distributions count instances that reported within the last 30 days** — one vote per instance, from its newest ping in that window. The "reports over time" history on the page covers a longer, 90-day window instead.
- **`schemaVersion` stays at `1` for this change.** The page publishes aggregates computed from data instances were already sending; the payload itself is unchanged, so no instance is asked for fresh consent. Deciding whether that is acceptable is exactly what this section is for — an admin who already agreed to `schemaVersion: 1` should judge that consent against what is written here, not against a version bump that didn't happen.
