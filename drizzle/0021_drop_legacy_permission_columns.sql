...
CREATE TABLE `__new_calendar_access_tokens` (
	...
	FOREIGN KEY (`bundle_id`) REFERENCES `calendar_permission_bundles`(`id`) ON UPDATE no action ON DELETE cascade,
...
ALTER TABLE `calendars` DROP COLUMN `guest_permission`;--> statement-breakpoint
ALTER TABLE `calendars` DROP COLUMN `allow_self_signup`;